...
show_help () {
cat << USAGE
usage: $0 [ -P POD-NAME-PREFIX ] [ -S NAMESPACE ] [ -F SOURCE-PATH ] [ -T DESTINATION-PATH ] 
       [ -c CLEARANCE-OR-NOT ]
    -p : Specify the prefix of pod name.
...
POD=$(kubectl -n ${NAMESPACE} get pod | grep ${POD} | awk -F ' ' '{print $1}')
echo $POD
FLAG=0
while (( ${FLAG} != 1 )); do
  if kubectl get po ${POD} | awk -F ' ' '{print $3}' | grep Running; then
    FLAG=1
    echo OKAY
  else
    sleep 1
